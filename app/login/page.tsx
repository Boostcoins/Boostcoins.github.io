  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6" style={{ background: 'var(--bg)' }}>
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <h1 className="text-[28px] font-bold tracking-tight mb-1" style={{ color: 'var(--dark)' }}>welcome back</h1>
        <p className="text-[14px] mb-10" style={{ color: 'var(--muted)' }}>log in to manage your agents.</p>

        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {[
            { label: 'username', key: 'username', type: 'text', placeholder: 'satoshi', auto: 'username' },
            { label: 'password', key: 'password', type: 'password', placeholder: '••••••••', auto: 'current-password' },
          ].map((f) => (
            <div key={f.key}>
        ...
        <p className="text-[13px] text-center mt-8" style={{ color: 'var(--muted)' }}>
          ...
          <Link href="/register" style={{ color: 'var(--blue)', fontWeight: 600 }}>get started</Link>
        </p>
      </div>
    </div>
  )
